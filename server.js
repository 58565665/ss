...
// ==========================================
// FIX: COMPLETAMENTO LOOP GIOCO E SERVER
// ==========================================
...
function safeGameTick() {
  try {
    ...
    game.lastTickError = String(err && err.message ? err.message : err);
  }
}

// Avvia il loop di gioco (di default a 40ms)
const tickRate = (CONFIG && CONFIG.SERVER && CONFIG.SERVER.TICK_RATE) ? CONFIG.SERVER.TICK_RATE : 40;
setInterval(safeGameTick, tickRate);

// Heartbeat di Render: pinga i client ogni 30 secondi. 
// Render chiude le connessioni se non c'è traffico per più di 55 secondi.
const pingInterval = setInterval(() => {
  wss.clients.forEach((ws) => {
    if (ws.isAlive === false) {
      if (socketPlayers.has(ws)) {
        const p = socketPlayers.get(ws);
        if (p) game.removePlayer(p.id);
        socketPlayers.delete(ws);
      }
      return ws.terminate();
    }
...
// Messa in ascolto del server (Render utilizza la porta definita da process.env.PORT)
const PORT = process.env.PORT || 10000;
server.listen(PORT, () => {
  console.log(`[ZERO LEGEND BACKEND] Server avviato con successo sulla porta ${PORT}`);
  console.log(`[ZERO LEGEND BACKEND] Endpoint Auth PHP: ${AUTH_VERIFY_URL}`);
});
