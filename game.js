...
const CONFIG = {
  ...
  BATTLE_ROYALE: {
    START_RADIUS_FACTOR: 0.5, // fraction of world width used as starting play-area radius
    MIN_RADIUS: 260,
    SHRINK_INTERVAL_MS: 20000, // how often the ring steps inward
    SHRINK_STEP_FACTOR: 0.82,  // multiplier applied to the radius on each step
    STORM_DPS: 6,              // damage/sec dealt outside the safe ring
  },
};
...
class World {
  constructor() {
    ...
    // Battle royale shrinking ring. Disabled by default; toggled via admin.startBattleRoyale/stopBattleRoyale.
    this.battleRoyale = {
      active: false,
      center: { x: CONFIG.WORLD.WIDTH / 2, y: CONFIG.WORLD.HEIGHT / 2 },
      radius: CONFIG.WORLD.WIDTH * CONFIG.BATTLE_ROYALE.START_RADIUS_FACTOR,
      nextShrinkAt: 0,
      startedAt: 0,
    };
  }
  ...
  spawnPowerup() {
    const types = ['virus', 'speed', 'mass', 'invisible', 'magnet', 'shield', 'teleport'];
  ...
  initZones() {
    ...
    const sr = CONFIG.ZONES.SAFE_RADIUS;
    for (let i = 0; i < CONFIG.ZONES.SAFE_COUNT; i++) {
      this.zones.push({
        id: uid(),
        x: rand(sr, CONFIG.WORLD.WIDTH - sr),
        y: rand(sr, CONFIG.WORLD.HEIGHT - sr),
        r: sr,
        kind: 'safe',
      });
    }
...
class Player {
  constructor(id, name, isBot = false) {
    ...
    this.lifetime = { kills: 0, deaths: 0, coinsEarned: 0, massEaten: 0 };
...
class GameServer {
  ...
  flushSeason(force = false) {
    ...
    for (const p of this.world.players.values()) {
      ...
      rec.achievementsClaimed = p.achievementsClaimed;
      rec.lifetime = p.lifetime;
    }
  ...
  addPlayer(name, isBot = false, team = null, auth = null) {
    ...
    if (saved && typeof saved === 'object') {
      ...
      p.achievementsClaimed = Array.isArray(saved.achievementsClaimed) ? saved.achievementsClaimed.slice(0, 200) : [];
      p.lifetime = saved.lifetime && typeof saved.lifetime === 'object'
        ? { kills: Number(saved.lifetime.kills) || 0, deaths: Number(saved.lifetime.deaths) || 0, coinsEarned: Number(saved.lifetime.coinsEarned) || 0, massEaten: Number(saved.lifetime.massEaten) || 0 }
        : { kills: 0, deaths: 0, coinsEarned: 0, massEaten: 0 };
    }
  ...
  eliminatePlayer(victim, killer = null, reason = 'pvp') {
    ...
    this.addLifetimeStat(victim, 'deaths', 1);
    this.checkAchievements(victim);
  ...
  handlePvpKill(killer, victim, mass, reason = 'pvp') {
    ...
    this.addLifetimeStat(killer, 'kills', 1);
    this.checkAchievements(killer);
    return true;
  }
  ...
  addCoins(p, amount, reason = 'reward') {
    ...
    p.coins = clamp(p.coins + gained, 0, 100000000); p.shopHistory.push({ type:'earn', amount:gained, reason, at:Date.now() }); if (p.shopHistory.length > 40) p.shopHistory.shift(); this.markSeasonDirty();
    if (!String(reason).startsWith('achievement:')) this.addLifetimeStat(p, 'coinsEarned', gained);
    return gained;
  }
  ...
  // Pool of possible daily quests. Each day, 3 are picked deterministically
  // from the date (same picks for everyone, but different pool than yesterday).
  static QUEST_POOL = [
    { id: 'kills3', label: '3 uccisioni', type: 'kills', goal: 3, reward: 150 },
    { id: 'kills10', label: '10 uccisioni', type: 'kills', goal: 10, reward: 450 },
    { id: 'kills20', label: '20 uccisioni', type: 'kills', goal: 20, reward: 800 },
    { id: 'coins1000', label: 'Guadagna 1000 ZC', type: 'coins', goal: 1000, reward: 180 },
    { id: 'coins3000', label: 'Guadagna 3000 ZC', type: 'coins', goal: 3000, reward: 400 },
    { id: 'play5', label: '5 minuti online', type: 'plays', goal: 5, reward: 120 },
    { id: 'play20', label: '20 minuti online', type: 'plays', goal: 20, reward: 350 },
    { id: 'eat40', label: 'Mangia massa per 40 volte', type: 'eats', goal: 40, reward: 200 },
  ];

  dailyQuestSeed(dateStr) { let h = 0; for (let i = 0; i < dateStr.length; i++) h = (h * 31 + dateStr.charCodeAt(i)) >>> 0; return h; }

  todaysQuestDefs() {
    const d = new Date().toISOString().slice(0, 10);
    const pool = GameServer.QUEST_POOL;
    let seed = this.dailyQuestSeed(d);
    const picks = [];
    ...
    return { date: d, defs: picks };
  }

  refreshQuests(p) {
    if (!p) return [];
    const { date, defs } = this.todaysQuestDefs();
    if (p.questState.date !== date) {
      p.questState = { date, kills: 0, coins: 0, plays: 0, eats: 0, claimed: [], questIds: defs.map((q) => q.id) };
    }
    return p.questState;
  }
  ...
  getQuests(p) {
    ...
    const { defs } = this.todaysQuestDefs();
    return defs.map((q) => {
      const progress = p.questState[q.type] || 0;
      return { id: q.id, label: q.label, goal: q.goal, reward: q.reward, progress, complete: progress >= q.goal, claimed: (p.questState.claimed || []).includes(q.id) };
    });
  }
  ...
  addQuestProgress(p, type, amount = 1) {
    ...
    if (type === 'coins') p.questState.coins = (p.questState.coins || 0) + amount;
    if (type === 'play') p.questState.plays = (p.questState.plays || 0) + amount;
    if (type === 'eat') p.questState.eats = (p.questState.eats || 0) + amount;
    this.markSeasonDirty();
  }

  claimQuest(p, questId) {
    const q = this.getQuests(p).find((x) => x.id === questId);
    if (!q || !q.complete || q.claimed) return false;
    p.questState.claimed.push(q.id);
    this.addCoins(p, q.reward, 'quest:' + q.id);
    this.checkAchievements(p);
    return true;
  }

  // =========================
  // ACHIEVEMENT (permanent, tracked on lifetime stats — never reset)
  // =========================
  static ACHIEVEMENT_DEFS = [
    { id: 'first_blood', label: 'Prima uccisione', stat: 'kills', goal: 1, reward: 100 },
    { id: 'hunter_25', label: '25 uccisioni totali', stat: 'kills', goal: 25, reward: 300 },
    { id: 'hunter_100', label: '100 uccisioni totali', stat: 'kills', goal: 100, reward: 1200 },
    { id: 'survivor_10', label: 'Sopravvivi a 10 morti', stat: 'deaths', goal: 10, reward: 100 },
    { id: 'tycoon_5000', label: 'Guadagna 5000 ZC nel tempo', stat: 'coinsEarned', goal: 5000, reward: 250 },
    { id: 'tycoon_20000', label: 'Guadagna 20000 ZC nel tempo', stat: 'coinsEarned', goal: 20000, reward: 900 },
    { id: 'glutton_100', label: 'Assorbi massa 100 volte', stat: 'massEaten', goal: 100, reward: 300 },
  ];

  addLifetimeStat(p, key, amount = 1) {
    if (!p) return;
    if (!p.lifetime) p.lifetime = { kills: 0, deaths: 0, coinsEarned: 0, massEaten: 0 };
    p.lifetime[key] = (p.lifetime[key] || 0) + amount;
    this.markSeasonDirty();
  }
  ...
  getAchievements(p) {
    ...
    const claimed = p.achievementsClaimed || [];
    return GameServer.ACHIEVEMENT_DEFS.map((a) => {
      const progress = lifetime[a.stat] || 0;
      return { id: a.id, label: a.label, goal: a.goal, reward: a.reward, progress, complete: progress >= a.goal, claimed: claimed.includes(a.id) };
    });
  }

  // Auto-claims any newly-completed achievement and credits its reward.
  // Called after events that move lifetime stats (kills, deaths, coin gains).
  checkAchievements(p) {
    if (!p) return [];
    if (!p.achievementsClaimed) p.achievementsClaimed = [];
    const newlyClaimed = [];
    for (const a of this.getAchievements(p)) {
      if (a.complete && !a.claimed) {
        p.achievementsClaimed.push(a.id);
        this.addCoins(p, a.reward, 'achievement:' + a.id);
        this.queueEvent(p, 'achievement', { id: a.id, label: a.label, reward: a.reward });
        newlyClaimed.push(a.id);
      }
    }
    return newlyClaimed;
  }
      ...
      for (let gx = minGX; gx <= maxGX; gx++) {
        for (let gy = minGY; gy <= maxGY; gy++) {
          ...
          for (const j of bucket) {
            ...
            if (this.inSafeZone(a.c) || this.inSafeZone(b.c)) continue;
            ...
            if (eatA && (!eatB || a.c.mass >= b.c.mass)) {
              ...
              this.addQuestProgress(a.p, 'eat', 1);
              this.addLifetimeStat(a.p, 'massEaten', 1);
              this.checkAchievements(a.p);
            } else {
              ...
              this.addQuestProgress(b.p, 'eat', 1);
              this.addLifetimeStat(b.p, 'massEaten', 1);
              this.checkAchievements(b.p);
              break;
            }
          }
        }
  ...
  applyPowerup(player, cell, pu, now = Date.now()) {
    switch (pu.type) {
      ...
      case 'teleport': this.teleportPlayer(player); break;
    }
  }

  // Teleports every cell of the player to a fresh safe spot, keeping the
  // cells' relative offsets so a split player doesn't get merged/collapsed.
  teleportPlayer(player) {
    if (!player || !player.cells.length) return false;
    const dest = this.findSafeSpawn(player);
    const origin = player.center;
    const dx = dest.x - (origin ? origin.x : dest.x);
    const dy = dest.y - (origin ? origin.y : dest.y);
    for (const cell of player.cells) {
      cell.x = clamp(cell.x + dx, cell.radius, CONFIG.WORLD.WIDTH - cell.radius);
      cell.y = clamp(cell.y + dy, cell.radius, CONFIG.WORLD.HEIGHT - cell.radius);
      cell.vx = 0; cell.vy = 0;
      cell.splitVx = 0; cell.splitVy = 0; cell.splitUntil = 0;
    }
    this.queueEvent(player, 'status', { name: 'teleport' });
    return true;
  }
  ...
  // True if a cell's center currently sits inside any 'safe' zone.
  inSafeZone(cell) {
    if (!cell) return false;
    for (const z of this.world.zones) {
      if (z.kind === 'safe' && dist2(cell, z) < z.r * z.r) return true;
    }
    return false;
  }
  ...
  resolveZones(dt) {
    for (const player of this.world.players.values()) {
      ...
      if (!center) continue;
      if (this.inSafeZone(center)) continue;
      for (const z of this.world.zones) {
  ...
  // =========================
  // BATTLE ROYALE (shrinking ring)
  // =========================
  startBattleRoyale() {
    const br = this.world.battleRoyale;
    br.active = true;
    br.center = { x: CONFIG.WORLD.WIDTH / 2, y: CONFIG.WORLD.HEIGHT / 2 };
    br.radius = CONFIG.WORLD.WIDTH * CONFIG.BATTLE_ROYALE.START_RADIUS_FACTOR;
    br.startedAt = Date.now();
    br.nextShrinkAt = Date.now() + CONFIG.BATTLE_ROYALE.SHRINK_INTERVAL_MS;
    this.world.announcements.push({ id: uid(), text: 'Battle Royale avviata! Il cerchio si restringerà nel tempo.', at: Date.now() });
    return true;
  }

  stopBattleRoyale() {
    this.world.battleRoyale.active = false;
    return true;
  }

  adminStartBattleRoyale() { return this.startBattleRoyale(); }
  adminStopBattleRoyale() { return this.stopBattleRoyale(); }

  battleRoyaleTick(now, dt) {
    const br = this.world.battleRoyale;
    if (!br.active) return;
    if (now >= br.nextShrinkAt && br.radius > CONFIG.BATTLE_ROYALE.MIN_RADIUS) {
      br.radius = Math.max(CONFIG.BATTLE_ROYALE.MIN_RADIUS, br.radius * CONFIG.BATTLE_ROYALE.SHRINK_STEP_FACTOR);
      br.nextShrinkAt = now + CONFIG.BATTLE_ROYALE.SHRINK_INTERVAL_MS;
      this.world.announcements.push({ id: uid(), text: 'Il cerchio si sta restringendo!', at: now });
    }
    for (const player of this.world.players.values()) {
      if (player.dead) continue;
      const center = player.center;
      if (!center) continue;
      if (dist2(center, br.center) <= br.radius * br.radius) continue;
      // Outside the ring: storm damage, ignores shield (like the hazard zone).
      for (const cell of player.cells) {
        cell.mass = Math.max(3, cell.mass - CONFIG.BATTLE_ROYALE.STORM_DPS * dt);
      }
    }
    // Auto-declare a winner once only one player remains alive with cells.
    const alive = [...this.world.players.values()].filter((p) => !p.dead && p.cells.length);
    if (alive.length === 1 && this.world.players.size > 1) {
      const winner = alive[0];
      this.world.announcements.push({ id: uid(), text: `${winner.name} ha vinto la Battle Royale!`, at: now });
      this.queueEvent(winner, 'battle-royale-win', {});
      this.stopBattleRoyale();
    }
  }
  ...
  snapshotFor(viewer) {
    ...
    const out = {
      ...
      battleRoyale: this.world.battleRoyale,
